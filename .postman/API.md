...
| ------ | --------------------- | ------------------------------------------------------------------------ | ------------- |
| GET    | `/api/resources`      | `?type=&status=&q=&location=&minCapacity=&maxCapacity=&sortBy=&sortDir=` | Authenticated |
...
`sortBy`: `name | capacity | location | type | status` (defaults to `name`)  
`sortDir`: `asc | desc` (defaults to `asc`)  
`q`: case-insensitive keyword match on resource name or location.
